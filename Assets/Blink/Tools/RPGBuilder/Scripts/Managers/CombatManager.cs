    ...
    public class CombatManager : MonoBehaviour
    {
        ...
        public void HandleCombatAction(CombatNode cbtNode)
        {
            ...
            if (cbtNode == playerCombatNode)
            {
                ...
                if(!playerCombatNode.appearanceREF.isShapeshifted) playerCombatNode.appearanceREF.HandleAnimatorOverride();
            }
        ...
        public void ExecuteDOTPulse(CombatNode casterInfo, CombatNode targetInfo, RPGEffect effect, int effectRank, int curStack, RPGAbility.RPGAbilityRankData rankREF)
        {
            ...
            if(!CanHitNode(casterInfo, targetInfo)) return;
        ...
        {
            if (playerCombatNode == null || casterInfo == null || targetInfo == null || targetInfo.dead || !CanHitNode(casterInfo, targetInfo)) return;
        ...
        private bool CanHitNode (CombatNode casterInfo, CombatNode targetInfo)
        {
            bool result =
                (targetInfo.nodeType != CombatNode.COMBAT_NODE_TYPE.mob &&
                 targetInfo.nodeType != CombatNode.COMBAT_NODE_TYPE.pet) ||
                !targetInfo.agentREF.IsAttackerTooFar(casterInfo);
            
            if (!result)
            {
                HandleCombatTextTrigger(casterInfo, targetInfo, 0, 0, "EVADE");
            }
            return result;
        }
            ...
            CombatNode targetInfo, RPGAbility.RPGAbilityRankData rankREF, float delay)
        {
            ...
            switch (effect.effectType)
            {
                case RPGEffect.EFFECT_TYPE.InstantDamage:
                    if(!CanHitNode(casterInfo, targetInfo)) yield break;
